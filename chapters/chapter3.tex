...
\end{align*}
where the adjoint variable $\bm{\lambda}\in\mathcal{C}^1(\mathcal{T})^r$ has the same dimension as the state variable $\hat{\mathbf{q}}(\cdot)$. The adjoint equation for $\bm{\lambda}(\cdot)$ is derived by setting\\
