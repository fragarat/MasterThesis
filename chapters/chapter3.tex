...
$$
where $\tilde{\mathbf{q}}(t,\hat{\bm\theta})$ denotes the reduced predicted trajectory produced by our dynamical model (i.e.\ the solution of \eqref{eq:redopinf}). Since $\tilde{\mathbf{q}}(t,\hat{\bm\theta})$ is generated from the discrete “ground-truth” snapshots $\{\mathbf{q}_{\mathrm{true}}(t_i)\}$, we need only interpolate those $\mathbf{q}_{\mathrm{true}}(t_i)$ values in time.
...
\begin{definition} 
    ...
    \begin{equation}
        \ell: \mathcal{C}^1(\mathcal{T})^r  \to \mathbb{R}\,,\quad \hat{\mathbf{q}}(\cdot) \mapsto \int_0^T \Big\|\hat{\mathbf{q}}(t) - \hat{\mathbf{q}}_\mathrm{true}(t)\Big\|_2^2~\dd t = \int_0^Tg(\hat{\mathbf{q}}(t),t)~\dd t,
        \label{eq:continuous_loss}
    \end{equation}
    where $g(\hat{\mathbf{q}}(t),t) \coloneqq \Big\|\hat{\mathbf{q}}(t) - \hat{\mathbf{q}}_\mathrm{true}(t)\Big\|_2^2$. We define the reduced loss functional as\\ 
    \begin{equation}
...
Based on the reduced loss functional \eqref{eq:continuous_reduced_loss} that considers the time integral of the RHS of our reduced dynamical system, the learning task is to find\\
\begin{equation}
  \hat{\bm{\theta}}^*
  \;=\;
  \min_{\hat{\bm{\theta}}}\;
...
$$\frac{\partial \ell}{\partial \hat{\mathbf{q}}}\Bigg\vert_{\hat{\mathbf{q}}(\cdot)=\tilde{\mathbf{q}}(\cdot, \hat{\bm{\theta}})}
...
Note that in the joint minimization \eqref{eq:opt_problem} the loss functional \(\ell\) depends only on the trajectory \(\hat{\mathbf{q}}(\cdot)\), measuring the misfit to the true data \(\hat{\mathbf{q}}_{\mathrm{true}}(\cdot)\), and does not depend explicitly on \(\hat{\bm{\theta}}\). Consequently, when we write\\
\[
...
\begin{align}\label{eq:ltildeder}
  ...
  \biggr).\notag
\end{align}
Then, we enforce stationarity of the derivative, $\dfrac{\partial\mathscr{L}}{\partial\hat{\mathbf{q}}}\Bigg\vert_{\hat{\mathbf{q}}(\cdot) = \tilde{\mathbf{q}}(\cdot, \hat{\bm{\theta}} ) } = \bm{0}$, condition that will provide the adjoint equations to determine $\bm{\lambda}$, ensuring\\
\begin{equation}
  \label{eq:equal_grad}
  \frac{\mathrm{d}\tilde{\ell}(\hat{\bm{\theta}})}{\mathrm{d}\hat{\bm{\theta}}}
...
\end{align*}
We recognize all the terms multiplying the sensitivity $\displaystyle\dv{\tilde{\mathbf{q}}}{\hat{\bm{\theta}}}$ as the expression $\displaystyle\pdv{\mathscr{L}}{\tilde{\mathbf{q}}}$ which we make zero together with $\bm{\lambda}(T)$, since $\bm{\lambda}$ arbitrary and \eqref{eq:lambdaeq} holds for any value of $\bm{\lambda}$.
...
\begin{align*}
    \mathscr{L}(\hat{\mathbf{q}}(\cdot),\hat{\bm{\theta}},\bm{\lambda}(\cdot)) &= \ell(\hat{\mathbf{q}}(\cdot)) - \int_0^T \bm{\lambda}(t)^{\top}\boldsymbol{c}(\hat{\mathbf{q}}(t),\hat{\bm{\theta}})~\mathrm{d}t - \bm{\lambda}(0)^{\top}(\hat{\mathbf{q}}(0)-\hat{\mathbf{q}}_0)\\
    &= \int_0^T g\bigl(\hat{\mathbf{q}}(t),t \bigl)~\mathrm{d}t \\
    &- \int_0^T \bm{\lambda}(t)^{\top}\bigl(\dot{\hat{\mathbf{q}}}(t) - \hat{\mathbf{c}} - \hat{\mathbf{A}}\hat{\mathbf{q}}(t) - \hat{\mathbf{H}}(\hat{\mathbf{q}}(t)\otimes \hat{\mathbf{q}}(t)) - \hat{\mathbf{B}}\mathbf{u}(t)\bigr)~\mathrm{d}t \\
    &-\bm{\lambda}(0)^{\top}(\hat{\mathbf{q}}(0)-\hat{\mathbf{q}}_0),
\end{align*}
where the adjoint variable $\bm{\lambda}(\cdot)\in\mathcal{C}^1(\mathcal{T})^r$ has the same dimension as the state variable $\hat{\mathbf{q}}(\cdot)$. The adjoint equation for $\bm{\lambda}(\cdot)$ is derived by setting\\
$$ \bm{D}_{\hat{\mathbf{q}}} \mathscr{L}(\hat{\mathbf{q}},\hat{\bm{\theta}},\bm{\lambda};\mathbf{v}) = \dv{}{\varepsilon}[\mathscr{L}(\hat{\mathbf{q}}+\varepsilon \mathbf{v},\bm{\lambda,\hat{\bm{\theta}}})]_{\varepsilon=0} = \bm{0}~~~\forall~\mathbf{v}\in\mathcal{C}^1(\mathcal{T})^r,$$a directional (Gâteaux) derivative. Noting first that\\
\begin{align*}
    \dv{}{\varepsilon}[\mathcal{\ell}(\hat{\mathbf{q}}+\varepsilon \mathbf{v},\hat{\bm{\theta}})]_{\varepsilon=0} &= \int_0^T \dv{}{\varepsilon}[g(\hat{\mathbf{q}}+\varepsilon \mathbf{v},t)]_{\varepsilon=0}~\mathrm{d}t = \int_0^T \nabla_{\hat{\mathbf{q}}}~g(\hat{\mathbf{q}},t)^{\top}\mathbf{v}~\mathrm{d}t,\\
    \dv{}{\varepsilon}[\boldsymbol{c}(\hat{\mathbf{q}}+\varepsilon \mathbf{v},\hat{\bm{\theta}})]_{\varepsilon=0} &= \dot{\mathbf{v}} - \hat{\mathbf{A}}\mathbf{v} - 2\hat{\mathbf{H}}[\hat{\mathbf{q}}\otimes \mathbf{v}],
\end{align*}
...
\end{equation*}
This linear (but non-autonomous) system can be solved with `backward time-integration' given a trajectory $\hat{\mathbf{q}}(t)$. The system can be written in a clearer linear form (with respect to $\bm{\lambda}(t)$) as\\
\begin{align}
...
An alternative approach to proving Theorem \ref{thm:adjoint_method}, proposed by the thesis supervisor,  yields the following proof. We include it here as it offers additional insight into the problem.

We start from the \emph{primal} sensitivity equations for the state solution $\tilde{\mathbf{q}}(t,\hat{\bm{\theta}})$ and parameters $\hat{\theta}_i$ for $i=1,\dots,d$:\\
$$\dot{\tilde{\mathbf{q}}}(t)
...
\subsection*{Theoretical and Empirical Validation}
...
\begin{itemize}
  ...
  the first-order stationarity conditions with respect to~$\hat{\mathbf{q}}$ yield the adjoint ODE that ensures the computed gradients respect the dynamical constraints.
    ...
    \item \textbf{Duality}: The adjoint variable $\bm{\lambda}(t)$ has a fundamental interpretation as a sensitivity measure. Specifically, it quantifies how perturbations in the state vector $\hat{\mathbf{q}}(t)$ propagate through the system to affect the final loss. This duality perspective aligns with reverse-mode automatic differentiation in machine learning \cite{chen2018neural}, where gradients flow backward through computational graphs.

    \item \textbf{Consistency with Pontryagin's Maximum Principle.}  Defining the Hamiltonian of the optimal-control problem\\
...
denotes the right-hand side of the reduced-order model (ROM-OpInf) dynamics \eqref{eq:redopinf}, and\\
$$g:\mathbb{R}^r\times [0,T]\to \mathbb{R}$$ 
defines the integrand of the loss functional $\ell$ \eqref{eq:continuous_loss}. The efficient computation of these gradients is crucial for both the backward integration of the adjoint equation and for evaluating the total derivative of the reduced loss $\tilde\ell$. What follows is a detailed derivation of the expressions of each of these gradients.
...
\end{equation}
\end{definition}
Equation~\eqref{eq:L_smoothness} states that the gradient of $\mathcal{J}$ is Lipschitz continuous with constant~$L$.
